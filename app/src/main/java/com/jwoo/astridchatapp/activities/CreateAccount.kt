...
class CreateAccount : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCreateAccountnSave.setOnClickListener {
            ...
            if ((!TextUtils.isEmpty(displayName)) && (!TextUtils.isEmpty(emailAddress)) && (!TextUtils.isEmpty(
                    password
                ))
            ) {
            ...
            .addOnCompleteListener { task: Task<AuthResult> ->
                if (task.isSuccessful) {
                        ...
                        .addOnCompleteListener { task: Task<Void> ->
                            if (task.isSuccessful) {
                                SharedFunctions().Log(
                                    this,
                                    "AstridChatApp-CreateUser",
                                    "Account has been created. - $userId",
                                    false
                                )
                                ...
                                SharedFunctions().Log(
                                    this,
                                    "AstridChatApp-CreateUser",
                                    "FirebaseDatabase - Failed to create user account."
                                )
                            }
                        }
                } else {
                    SharedFunctions().Log(
                        this,
                        "AstridChatApp-CreateUser",
                        "FirebaseAuth - Failed to create user account."
                    )
                }
            }
    }
